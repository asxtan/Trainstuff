...
- **Trip** is a one-off lookup for any journey — pick any two stations and a
  departure time, and the matching train is flagged with its platform. Leave the
  destination blank for every departure from that station.
...
- **Platforms** are usually only assigned 10–20 minutes before departure, so a
  Trip lookup for later today can show `—` until closer to the time.
- National Rail publishes departure boards about **two hours ahead**. Ask Trip
  for a time beyond that and it shows the furthest board available and tells you
  when to check back.
